...
public class TexasHoldEmGame : IBaseGame<StandardCard>
{
  ...
  public bool SetPlayerStatus(string playerName, LyssiePlayerStatus status) {
    switch (status) {
      case (LyssiePlayerStatus.Active):
        return this.State.SetPlayerToActive(playerName);
      case (LyssiePlayerStatus.Spectator):
        return this.State.SetPlayerToSpectator(playerName);
      case (LyssiePlayerStatus.Afk):
        return this.State.SetPlayerToAFK(playerName);
      case (LyssiePlayerStatus.NotAfk):
        throw new NotImplementedException("Haven't implemented the not afk case lmao");
      default:
  ...
  public bool Call(string playerName) {
    return this.State.Call(playerName);
